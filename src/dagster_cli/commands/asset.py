    ...
    try:
        ...
        if json_output:
            ...
            for asset in assets:
                ...
                asset_key_str = "/".join(asset_key) if isinstance(asset_key, list) else str(asset_key)
...
@app.command()
def view(
    asset_key: str = typer.Argument(..., help="Asset key (e.g., 'my_asset' or 'prefix/my_asset')"),
    profile: Optional[str] = typer.Option(
        None,
        "--profile", "-p",
        help="Use specific profile"
        ...
        if json_output:
            ...
            if deps:
                ...
                for dep in deps:
                    ...
                    dep_key_str = "/".join(dep_key) if isinstance(dep_key, list) else str(dep_key)
...
def materialize(
    ...
    try:
        ...
        print_success(f"Materialization submitted successfully!")
...
def health(
    all_assets: bool = typer.Option(
        False,
        "--all", "-a",
        help="Show all assets (default: unhealthy only)"
    ),
    stale_hours: int = typer.Option(
        24,
        "--stale-hours",
        help="Hours before asset is considered stale"
    ),
    group: Optional[str] = typer.Option(
        None,
        "--group", "-g",
    ...
    try:
        ...
        # Prepare output
        all_assets_list = failed_assets + never_materialized + stale_assets + healthy_assets
        unhealthy_count = len(failed_assets) + len(never_materialized) + len(stale_assets)
        ...
        if json_output:
            output = {
                ...
                "assets": all_assets_list if all_assets else (failed_assets + never_materialized + stale_assets)
            }
            ...
            # Create table
            assets_to_show = all_assets_list if all_assets else (failed_assets + never_materialized + stale_assets)
            
            if assets_to_show:
                console.print(f"\n[bold]Asset Details[/bold]")
                if not all_assets:
                    console.print(f"[dim]Showing {unhealthy_count} unhealthy assets (use --all to see all)[/dim]")
