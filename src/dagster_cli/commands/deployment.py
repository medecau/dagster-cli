...
def deployment_callback(ctx: typer.Context):
...
def _format_deployment_name(deployment: dict) -> tuple[str, str]:
    """Return (display_name, deployment_type) for a deployment dict."""
...
def test(
    ...
    try:
        with create_spinner(f"Testing deployment '{deployment_name}'...") as (
            progress,
            task,
        ):
            try:
